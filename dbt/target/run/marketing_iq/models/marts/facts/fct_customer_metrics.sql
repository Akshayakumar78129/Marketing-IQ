...
create or replace transient table CLIENT_RARE_SEEDS_DB.PUBLIC_analytics.fct_customer_metrics
...
meta_spend as (
    ...
    from CLIENT_RARE_SEEDS_DB.META_ADS.basic_campaign
    group by spend_month
),
...
select
    md5(cast(coalesce(cast(c.cohort_month as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as customer_metrics_sk,
    c.cohort_month,
