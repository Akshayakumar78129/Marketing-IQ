...
create or replace transient table CLIENT_RARE_SEEDS_DB.PUBLIC_analytics.fct_regional_revenue
...
select
    md5(cast(coalesce(cast(order_date as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(country as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(region as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(city as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as regional_revenue_sk,
    'klaviyo' as platform,
    order_date,
