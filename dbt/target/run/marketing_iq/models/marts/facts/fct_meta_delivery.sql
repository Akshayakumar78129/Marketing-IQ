...
create or replace transient table CLIENT_RARE_SEEDS_DB.PUBLIC_analytics.fct_meta_delivery
...
with device_data as (
    ...
    from CLIENT_RARE_SEEDS_DB.META_ADS.delivery_device
...
platform_data as (
    select
        ...
        frequency,
        _fivetran_synced as last_synced
    from CLIENT_RARE_SEEDS_DB.META_ADS.delivery_platform
...
select
    md5(cast(coalesce(cast(account_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(date_day as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(dimension_type as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(dimension_value as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as delivery_sk,
    'meta' as platform,
    account_id,
    date_day,
    dimension_type,
