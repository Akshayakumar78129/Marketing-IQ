...
with ad_group_stats as (
    ...
    from CLIENT_RARE_SEEDS_DB.GOOGLE_ADS.ad_group_stats
    
    where _fivetran_synced > (select max(last_synced) from CLIENT_RARE_SEEDS_DB.PUBLIC_analytics.fct_ad_group_performance)
    
    group by date, id, campaign_id, customer_id, device, ad_network_type
)

select
    md5(cast(coalesce(cast(date_day as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(ad_group_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(device as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(ad_network_type as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as ad_group_performance_sk,
