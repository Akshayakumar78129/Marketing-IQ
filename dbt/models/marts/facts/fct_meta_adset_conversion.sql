{{
    config(
        ...
        unique_key='meta_adset_conversion_sk',
        tags=['facts', 'meta', 'conversions']
    )
}}

/*
    Meta Ads Ad Set-Level Conversion Fact Table
    Contains conversion actions (purchases, add_to_cart, etc.) by ad set
    Source: META_ADS.BASIC_AD_SET_ACTIONS

    NOTE: This table has FULL HISTORICAL DATA (2.8+ years) unlike ad-level
    conversions which are limited to ~30 days by Meta's API.
*/

with adset_actions as (
    select
    ...
    from {{ source('meta_ads', 'basic_ad_set_actions') }}
    {% if is_incremental() %}
...
pivoted as (
    ...
    from adset_actions
    group by adset_id, date_day
)

select
    {{ dbt_utils.generate_surrogate_key(['adset_id', 'date_day']) }} as meta_adset_conversion_sk,
    'meta' as platform,
    adset_id,
    date_day,
